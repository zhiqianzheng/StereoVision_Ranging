...
一个基于立体视觉（stereo vision）实现目标距离测量（ranging / depth estimation）的项目。该仓库包含相机标定、立体校正、视差图计算及基于视差的深度/距离估算的代码与说明，适合用于机器人导航、无人机避障、测距实验和教学演示。

> 注意：下面的说明包含常见工作流和示例命令。根据仓库实际文件名/脚本（例如 `calibrate.py`、`rectify.py`、`compute_disparity.py`、`estimate_depth.py` 等）做相应替换。
